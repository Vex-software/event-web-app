...
function Pass({ label, name, ...otherProps }) {
  ...
  return (
    ...
    <div className={`relative ${pad}`}>
      ...
      {inputValue.trim() !== "" && (
        <button onClick={() => setType(!type)} className='p-0 px-2 text-left absolute top-[2.3rem] right-[1rem] rounded font-bold font-["Jost"] text-[#000] text-sm'>{type ? 'Hide' : 'Show'}</button>
      )}
    </div>
